...
function pushToCollection<T>(item: T, collection: T[]): T[] {
    const itemIsSameTypeAsCollection = collection.every(element => typeof element === typeof item);
    if (itemIsSameTypeAsCollection) {
        collection.push(item);
        return collection;
    }

    // throw new Error("Hey, you are trying to push an item of different type in the collection");
}
